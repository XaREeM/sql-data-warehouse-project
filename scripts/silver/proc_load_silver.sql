...
==============================================
Script purpose:
	This script loads data into the 'silver' schema from the already loaded 'bronze' schema, performing the following:
	- Truncate tables before loading data.
	- Inserting into 'silver' tables data that is laoded from 'bronze' layer
...
BEGIN
	...
	BEGIN TRY
		...
		CASE UPPER(TRIM(prd_line))
			...
			WHEN 'T' THEN 'Touring'
			ELSE 'N/A'
		END AS prd_line, -- Provide more desciptive values to the product line codes 
		CAST(prd_start_dt AS DATE) AS prd_start_dt,
		CAST(LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE) AS prd_end_dt -- Fetch end date from the start date of the next row decreasing one day
		...
		sls_prd_key,
		sls_cust_id,
		-- Another method for replacing incorrect date formats with 0 first then casting as DATE
